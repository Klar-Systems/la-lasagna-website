...
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,2600','--no-first-run',`--user-data-dir=/tmp/cdp-ts-${PORT}`]);
...
const e=`JSON.stringify((()=>{const b=document.querySelector('.fe-block-5ee92170caf33deb7e85');const big=b.querySelector('h1,h2,[style*=font-size]')||b;const out=[];b.querySelectorAll('*').forEach(el=>{const t=(el.textContent||'').trim();if(/4\\.8|400/.test(t)&&t.length<14){out.push({t:t.slice(0,8),fs:getComputedStyle(el).fontSize});}});return out.slice(0,6);})())`;
