...
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,1200','--no-first-run',`--user-data-dir=/tmp/cdp-cs-${PORT}`]);
...
await cdp('Page.navigate',{url:'http://127.0.0.1:8080/menu'});await new Promise(r=>setTimeout(r,5000));
const e=`JSON.stringify((()=>{const s=document.querySelector('script.TextAttributes-props');if(!s)return{none:1};const cs=getComputedStyle(s);const r=s.getBoundingClientRect();
return{display:cs.display,visibility:cs.visibility,rectH:Math.round(r.height),rectW:Math.round(r.width),parentText:(s.parentElement.textContent||'').replace(/\\s+/g,' ').trim().slice(0,40)};})())`;
