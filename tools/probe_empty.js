...
const CHROME='/Applications/Google Chrome.app/Contents/MacOS/Google Chrome';const PORT=9951;
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=1440,900','--no-first-run','--user-data-dir=/tmp/cdp-empty']);
...
const e=`JSON.stringify([...document.querySelectorAll('section.page-section')][0].querySelector('.fluid-engine').children ? [...[...document.querySelectorAll('section.page-section')][0].querySelector('.fluid-engine').children].map(b=>{const r=b.getBoundingClientRect();const cs=getComputedStyle(b);const txt=(b.textContent||'').trim();const hasMedia=!!b.querySelector('img,video,iframe');return {id:(b.className||'').toString().match(/fe-block-[0-9a-f]+/)?.[0]||'?', gridRow:cs.gridRow, h:Math.round(r.height), empty: txt.length===0 && !hasMedia };}):[])`;
