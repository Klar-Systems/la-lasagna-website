...
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,2600','--no-first-run',`--user-data-dir=/tmp/cdp-tp-${PORT}`]);
...
const e=`JSON.stringify((()=>{const h=document.querySelector('.sqs-block-content h1, .fluid-engine h1, [class*=scaled] h1, h1');if(!h)return{none:1};const cs=getComputedStyle(h);const fb=h.closest('.fe-block');return{feid:fb?(fb.className.match(/fe-block-[0-9a-zA-Z_]+/)||[])[0]:'?',lineHeight:cs.lineHeight,fontSize:cs.fontSize,text:(h.textContent||'').replace(/\\s+/g,' ').slice(0,40)};})())`;
