...
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,1400','--no-first-run',`--user-data-dir=/tmp/cdp-bp2-${PORT}`]);
...
const e=`JSON.stringify([...document.querySelectorAll('.fe-block')].filter(b=>{const tt=(b.textContent||'').replace(/\\s+/g,'').trim();return /(Lasagnas|Pizzas|Salads|Desserts|Drinks|Lunch)/.test(tt)&&tt.length<12&&b.getBoundingClientRect().top<950;}).map(b=>{const r=b.getBoundingClientRect();const cs=getComputedStyle(b);const a=b.querySelector('a,button');return{id:(b.className.match(/fe-block-[0-9a-f]+/)||[])[0],t:(b.textContent||'').replace(/\\s+/g,' ').trim().slice(0,12),left:Math.round(r.left),top:Math.round(r.top),w:Math.round(r.width),h:Math.round(r.h||r.height),gc:cs.gridColumn,gr:cs.gridRow};}))`;
