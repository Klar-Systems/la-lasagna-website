...
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,2600','--no-first-run',`--user-data-dir=/tmp/cdp-igl-${PORT}`]);
...
const e=`JSON.stringify((()=>{const ig=[...document.querySelectorAll('.sqs-block-button-element')].find(e=>/instagram/i.test(e.textContent||''));const lm=document.querySelector('.fe-block-1dfe7d7cffc243e71ef8 .sqs-block-button-element');const g=e=>{if(!e)return null;const cs=getComputedStyle(e);return{lh:cs.lineHeight,mh:cs.minHeight,h:cs.height,pt:cs.paddingTop,pb:cs.paddingBottom,disp:cs.display};};return{ig:g(ig),lm:g(lm)};})())`;
