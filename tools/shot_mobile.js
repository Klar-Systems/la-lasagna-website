...
const URL=process.argv[2];const OUT=process.argv[3];const PORT=+(process.argv[4]||9995);
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,2400','--no-first-run',`--user-data-dir=/tmp/cdp-m${PORT}`]);
...
fs.writeFileSync(OUT,Buffer.from(shot.data,'base64'));console.log('saved '+OUT);c.kill();process.exit(0);})();
