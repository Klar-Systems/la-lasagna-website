...
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,1400','--no-first-run',`--user-data-dir=/tmp/cdp-bp-${PORT}`]);
...
const e=`JSON.stringify([...document.querySelectorAll('.fe-block')].filter(b=>{const tt=(b.textContent||'').trim();return /^(Lasagnas|Pizzas|Salads|Desserts|Drinks|Lunch)$/.test(tt);}).map(b=>{const r=b.getBoundingClientRect();const cs=getComputedStyle(b);return{id:(b.className.match(/fe-block-[0-9a-f]+/)||[])[0],t:(b.textContent||'').trim(),left:Math.round(r.left),top:Math.round(r.top),w:Math.round(r.width),h:Math.round(r.height),gc:cs.gridColumn,gr:cs.gridRow};}))`;
