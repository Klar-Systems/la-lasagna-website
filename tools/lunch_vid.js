...
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,2600','--no-first-run',`--user-data-dir=/tmp/cdp-lv-${PORT}`]);
...
await cdp('Page.navigate',{url:'http://127.0.0.1:8080/lunch'});await new Promise(r=>setTimeout(r,5000));
const e=`JSON.stringify([...document.querySelectorAll('.fe-block')].filter(b=>b.querySelector('video')).map(b=>{const r=b.getBoundingClientRect();const v=b.querySelector('video');const vr=v.getBoundingClientRect();return{id:(b.className.match(/fe-block-[0-9a-f]+/)||[])[0],blockL:Math.round(r.left),blockW:Math.round(r.width),vidL:Math.round(vr.left),vidW:Math.round(vr.width),src:(v.currentSrc||v.src||'').split('/').pop().slice(0,20)};}))`;
