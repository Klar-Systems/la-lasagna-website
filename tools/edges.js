...
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,2600','--no-first-run',`--user-data-dir=/tmp/cdp-ed-${PORT}`]);
...
// big media/content blocks: report left & right edges to compare margins
const e=`JSON.stringify([...document.querySelectorAll('.fe-block')].map(b=>{const r=b.getBoundingClientRect();const img=b.querySelector('img,iframe,video');const tt=(b.textContent||'').replace(/\\s+/g,' ').trim();return{id:(b.className.match(/fe-block-[0-9a-f]+/)||[])[0],kind:img?img.tagName.toLowerCase():(tt?'text':'e'),left:Math.round(r.left),right:Math.round(390-r.right),w:Math.round(r.width),top:Math.round(r.top)};}).filter(b=>b.w>180&&b.kind!=='text').sort((a,b)=>a.top-b.top))`;
