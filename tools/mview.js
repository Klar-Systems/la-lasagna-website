...
const Y=+(process.argv[2]||2000);const OUT=process.argv[3];const PORT=9000+(process.pid%900);
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,844','--no-first-run',`--user-data-dir=/tmp/cdp-mv-${PORT}`]);
...
await cdp('Runtime.evaluate',{expression:`window.scrollTo(0,${Y})`});await new Promise(r=>setTimeout(r,800));
const shot=await cdp('Page.captureScreenshot',{});
fs.writeFileSync(OUT,Buffer.from(shot.data,'base64'));console.log('ok');c.kill();process.exit(0);})();
