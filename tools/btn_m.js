...
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,1400','--no-first-run',`--user-data-dir=/tmp/cdp-bm-${PORT}`]);
...
const e=`JSON.stringify((()=>{const grab=e=>{if(!e)return null;const cs=getComputedStyle(e);const r=e.getBoundingClientRect();return{text:(e.textContent||'').trim().slice(0,12),w:Math.round(r.width),display:cs.display,bg:cs.backgroundColor,border:cs.borderWidth,radius:cs.borderRadius,pad:cs.padding};};
const lm=document.querySelector('.fe-block-1dfe7d7cffc243e71ef8 .sqs-block-button-element');
const ig=[...document.querySelectorAll('.sqs-block-button-element')].find(e=>/instagram/i.test(e.textContent||''));
