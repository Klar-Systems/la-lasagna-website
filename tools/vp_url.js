...
const CHROME='/Applications/Google Chrome.app/Contents/MacOS/Google Chrome';const URL=process.argv[2];const SEL=process.argv[3];const OUT=process.argv[4];const PORT=9000+(process.pid%900);
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,844','--no-first-run',`--user-data-dir=/tmp/cdp-vu-${PORT}`]);
...
await cdp('Runtime.evaluate',{expression:`var e=document.querySelector('${SEL}');if(e)e.scrollIntoView({block:'center'});`});await new Promise(r=>setTimeout(r,600));
