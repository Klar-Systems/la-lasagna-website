...
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,2600','--no-first-run',`--user-data-dir=/tmp/cdp-mt-${PORT}`]);
...
await cdp('Page.navigate',{url:'http://127.0.0.1:8080/menu'});await new Promise(r=>setTimeout(r,4500));
const e=`JSON.stringify([...document.querySelectorAll('.fe-block')].map(b=>{const r=b.getBoundingClientRect();const tt=(b.textContent||'').replace(/\\s+/g,' ').trim();return{id:(b.className.match(/fe-block-[0-9a-zA-Z_]+/)||[])[0],t:tt.slice(0,30),top:Math.round(r.top),bottom:Math.round(r.bottom)};}).filter(b=>b.t.length>2&&b.top<900).sort((a,b)=>a.top-b.top))`;
