...
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,2600','--no-first-run',`--user-data-dir=/tmp/cdp-e2-${PORT}`]);
...
const ids={hero:'9afe7be36dc746df4a52',spritz:'132feefd1e18d9420227',book:'0080259be0871705e95c',video:'0bd9d632bada647776da'};
const e=`JSON.stringify((()=>{const o={};${Object.entries(ids).map(([k,v])=>`{const el=document.querySelector('.fe-block-${v}');if(el){const r=el.getBoundingClientRect();o['${k}']={L:Math.round(r.left),R:Math.round(390-r.right),w:Math.round(r.width)};}}`).join('')}return o;})())`;
