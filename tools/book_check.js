...
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,2600','--no-first-run',`--user-data-dir=/tmp/cdp-bk-${PORT}`]);
...
const e=`JSON.stringify((()=>{const b=document.querySelector('.fe-block-0080259be0871705e95c');if(!b)return{none:1};const r=b.getBoundingClientRect();const img=b.querySelector('img');const ir=img?img.getBoundingClientRect():null;return{block:{w:Math.round(r.width),h:Math.round(r.height)},img:ir?{w:Math.round(ir.width),h:Math.round(ir.height),left:Math.round(ir.left)}:null};})())`;
