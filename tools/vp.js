...
const CHROME='/Applications/Google Chrome.app/Contents/MacOS/Google Chrome';const SEL=process.argv[2];const OUT=process.argv[3];const PORT=9000+(process.pid%900);
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,844','--no-first-run',`--user-data-dir=/tmp/cdp-vp-${PORT}`]);
...
await cdp('Runtime.evaluate',{expression:`var e=document.querySelector('${SEL}');if(e)e.scrollIntoView({block:'center'});`});await new Promise(r=>setTimeout(r,700));
