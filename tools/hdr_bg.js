...
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,1400','--no-first-run',`--user-data-dir=/tmp/cdp-hb-${PORT}`]);
...
const e=`JSON.stringify((()=>{const h=document.querySelector('header');const cs=getComputedStyle(h);
return{bg:cs.backgroundColor,cls:h.className.slice(0,60),hasTransparentClass:/transparent|overlay/i.test(h.className)};})())`;
