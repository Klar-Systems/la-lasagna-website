...
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,1400','--no-first-run',`--user-data-dir=/tmp/cdp-hi-${PORT}`]);
...
const e=`JSON.stringify((()=>{const s=document.querySelector('section.page-section');const fe=s.querySelector('.fluid-engine');
const img=fe.querySelector('.section-background img')||s.querySelector('.section-background img');
const bg=s.querySelector('.section-background');
// the big hero image fe-block (132feefd from earlier)
const imgBlock=document.querySelector('.fe-block-132feefd1') || [...fe.querySelectorAll('.fe-block')].find(b=>b.querySelector('img')&&b.getBoundingClientRect().height>200);
return {feCols:getComputedStyle(fe).gridTemplateColumns.split(' ').length, feW:Math.round(fe.getBoundingClientRect().width),
  bg: bg?{w:Math.round(bg.getBoundingClientRect().width),left:Math.round(bg.getBoundingClientRect().left)}:null,
  imgBlock: imgBlock?{w:Math.round(imgBlock.getBoundingClientRect().width),left:Math.round(imgBlock.getBoundingClientRect().left),gc:getComputedStyle(imgBlock).gridColumn}:null};})())`;
