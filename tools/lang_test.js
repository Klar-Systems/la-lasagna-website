...
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,`--window-size=${W},900`,'--no-first-run',`--user-data-dir=/tmp/cdp-lt-${PORT}`]);
...
const e=`JSON.stringify((()=>{const dc=document.querySelectorAll('.language-picker-content');const out=[];dc.forEach(c=>out.push({html:(c.innerHTML||'').replace(/\\s+/g,' ').trim().slice(0,120),childCount:c.children.length}));
// also check if weglot global exists
return{pickerContents:out, weglotGlobal: typeof window.Weglot, wgInitialized: window.Weglot&&window.Weglot.initialized};})())`;
