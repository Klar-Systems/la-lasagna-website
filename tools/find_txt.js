...
const CHROME='/Applications/Google Chrome.app/Contents/MacOS/Google Chrome';const PORT=9000+(process.pid%900);const URL=process.argv[2];const NEEDLE=process.argv[3];
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,2600','--no-first-run',`--user-data-dir=/tmp/cdp-ft-${PORT}`]);
...
const e=`JSON.stringify((()=>{const b=[...document.querySelectorAll('.fe-block')].find(x=>(x.textContent||'').includes('${NEEDLE}'));if(!b)return{none:1};const sp=b.querySelector('span,p,h1,h2,h3,h4');const cs=getComputedStyle(sp||b);return{id:(b.className.match(/fe-block-[0-9a-zA-Z_]+/)||[])[0],color:cs.color,text:(b.textContent||'').replace(/\\s+/g,' ').trim().slice(0,40)};})())`;
