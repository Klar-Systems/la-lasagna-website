...
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,2600','--no-first-run',`--user-data-dir=/tmp/cdp-de-${PORT}`]);
...
await cdp('Page.navigate',{url:'http://127.0.0.1:8080/drinks'});await new Promise(r=>setTimeout(r,4500));
const e=`JSON.stringify([...document.querySelectorAll('.fe-block')].map(b=>{const r=b.getBoundingClientRect();const img=b.querySelector('img,iframe');const tt=(b.textContent||'').replace(/\\s+/g,' ').trim();return{id:(b.className.match(/fe-block-[0-9a-zA-Z_]+/)||[])[0],kind:img?'img':(tt?'text':'e'),t:tt.slice(0,18),L:Math.round(r.left),R:Math.round(390-r.right),w:Math.round(r.width),top:Math.round(r.top)};}).filter(b=>b.w>60&&b.top<1900).sort((a,b)=>a.top-b.top))`;
