...
const CHROME='/Applications/Google Chrome.app/Contents/MacOS/Google Chrome';const PORT=9100+(process.pid%800);const SID=process.argv[2];
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,2400','--no-first-run',`--user-data-dir=/tmp/cdp-sm${PORT}`]);
...
const e=`JSON.stringify([...document.querySelectorAll('section[data-section-id$="${SID}"] .fe-block')].map(b=>{const r=b.getBoundingClientRect();const m=b.querySelector('video,iframe,img');return{id:(b.className.match(/fe-block-[0-9a-f]+/)||[])[0],kind:m?m.tagName.toLowerCase():'text',left:Math.round(r.left),w:Math.round(r.width),top:Math.round(r.top),h:Math.round(r.h||r.height)};}).filter(b=>b.w>8&&b.kind!=='text'))`;
