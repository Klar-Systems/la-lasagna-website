...
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,1400','--no-first-run',`--user-data-dir=/tmp/cdp-c2-${PORT}`]);
...
const e=`JSON.stringify((()=>{
  const lm=document.querySelector('.fe-block-1dfe7d7cffc243e71ef8');
  const lmBtn=lm?lm.querySelector('a,button,.sqs-block-button-element'):null;
  const hdr=document.querySelector('header');
  const hero=document.querySelector('section.page-section');
  const g=e=>{const r=e.getBoundingClientRect();return{l:Math.round(r.left),t:Math.round(r.top),w:Math.round(r.width),h:Math.round(r.height),b:Math.round(r.bottom)};};
  return{learnBlock:lm?g(lm):null, learnBtn:lmBtn?g(lmBtn):null, header:hdr?{...g(hdr),pos:getComputedStyle(hdr).position}:null, heroTop:hero?Math.round(hero.getBoundingClientRect().top):null};
})())`;
