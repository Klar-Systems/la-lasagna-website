...
const CHROME='/Applications/Google Chrome.app/Contents/MacOS/Google Chrome';const PORT=9977;
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=1440,900','--no-first-run','--user-data-dir=/tmp/cdp-dp']);
...
await cdp('Page.enable');await cdp('Runtime.enable');await cdp('Page.navigate',{url:'http://127.0.0.1:8080/contact'});await new Promise(r=>setTimeout(r,4000));
const e=`JSON.stringify((()=>{const h=document.querySelector('.fe-block-916612e5e07cbc8f0eba').getBoundingClientRect();const ph=document.querySelector('.fe-block-523cda3f8fca79c5cf2b').getBoundingClientRect();return {gap:Math.round(ph.top-h.bottom)};})())`;
