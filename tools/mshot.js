...
const URL=process.argv[2];const OUT=process.argv[3];const PORT=9000+(process.pid%900);
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,2600','--no-first-run',`--user-data-dir=/tmp/cdp-ms-${PORT}`]);
...
await cdp('Page.navigate',{url:URL});await new Promise(r=>setTimeout(r,5000));
const shot=await cdp('Page.captureScreenshot',{captureBeyondViewport:true});
fs.writeFileSync(OUT,Buffer.from(shot.data,'base64'));console.log('ok '+OUT);c.kill();process.exit(0);})();
