...
const CHROME='/Applications/Google Chrome.app/Contents/MacOS/Google Chrome';const Y=+(process.argv[2]);const OUT=process.argv[3];const PORT=9000+(process.pid%900);
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,844','--no-first-run',`--user-data-dir=/tmp/cdp-mv2-${PORT}`]);
...
await cdp('Runtime.evaluate',{expression:`document.querySelector('section[data-section-id$="7d7058"]').scrollIntoView()`});await new Promise(r=>setTimeout(r,800));
