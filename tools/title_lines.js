...
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,2600','--no-first-run',`--user-data-dir=/tmp/cdp-tl-${PORT}`]);
...
const e=`JSON.stringify((()=>{const h=document.querySelector('section.page-section:first-of-type h1');if(!h)return{none:1};const cs=getComputedStyle(h);
// measure each word's rendered width vs container
const words=(h.textContent||'').trim().split(/\\s+/);
return{fontSize:cs.fontSize,lineHeight:cs.lineHeight,containerW:Math.round(h.getBoundingClientRect().width),text:(h.textContent||'').replace(/\\s+/g,' ').trim(),words:words};})())`;
