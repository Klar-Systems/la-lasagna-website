...
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,2600','--no-first-run',`--user-data-dir=/tmp/cdp-fl-${PORT}`]);
...
const e=`JSON.stringify((()=>{const b=[...document.querySelectorAll('.fe-block')].find(x=>(x.textContent||'').includes('Service in languages'));if(!b)return{none:1};const r=b.getBoundingClientRect();return{id:(b.className.match(/fe-block-[0-9a-f]+/)||[])[0],top:Math.round(r.top),mt:getComputedStyle(b).marginTop};})())`;
