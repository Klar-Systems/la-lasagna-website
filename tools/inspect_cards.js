...
const URL=process.argv[2];const PORT=9000+(process.pid%900);
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,2600','--no-first-run',`--user-data-dir=/tmp/cdp-ic-${PORT}`]);
...
const e=`JSON.stringify([...document.querySelectorAll('.fe-block')].map(b=>{const r=b.getBoundingClientRect();const cs=getComputedStyle(b);const img=b.querySelector('img');const tt=(b.textContent||'').replace(/\\s+/g,' ').trim();
  return{id:(b.className.match(/fe-block-[0-9a-f]+/)||[])[0],kind:img?'img':(tt?'text':'empty'),t:tt.slice(0,18),left:Math.round(r.left),top:Math.round(r.top),w:Math.round(r.width),h:Math.round(r.height),transform:cs.transform!=='none'?cs.transform.slice(0,30):'',gc:cs.gridColumn};}).filter(b=>b.w>8&&b.top>200&&b.top<3500))`;
