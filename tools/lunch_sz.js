...
const c=spawn(CHROME,['--headless=new',`--remote-debugging-port=${PORT}`,'--window-size=390,2600','--no-first-run',`--user-data-dir=/tmp/cdp-ls-${PORT}`]);
...
const e=`JSON.stringify([...document.querySelectorAll('.fe-block')].filter(b=>b.querySelector('video')).map(b=>{const r=b.getBoundingClientRect();return{id:(b.className.match(/fe-block-[0-9a-f]+/)||[])[0],w:Math.round(r.width),h:Math.round(r.height)};}))`;
