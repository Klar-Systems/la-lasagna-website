...
(function(){
  ...
  function buildMobile(){
    ...
    // build the FI/EN options inside a togglable menu
    ...
    c.innerHTML=''; c.appendChild(menu);
    // the picker trigger: tap the current-language row to expand the dropdown IN
    // PLACE (no navigation). Kill Squarespace's folder-open/href="#" default.
    var trigger=m.querySelector('a[data-folder-id], .current-language, [role="button"]') || m;
    trigger.addEventListener('click', function(ev){
      ev.preventDefault(); ev.stopPropagation();
      var willOpen=!menu.classList.contains('open'); closeMenu();
      if(willOpen){ menu.classList.add('open'); openMenu=menu; m.setAttribute('aria-expanded','true'); }
      else m.setAttribute('aria-expanded','false');
    });
  }
  function buildStyle(){
    var css=''
      ...
      /* mobile dropdown: flows in place under the picker (not absolute), centered */
      +'.lala-lang-menu-mobile{position:static;display:none;margin-top:8px;text-align:center;}'
      +'.lala-lang-menu-mobile.open{display:block;}'
      +'.lala-lang-menu-mobile .lala-lang-opt{text-align:center;}';
